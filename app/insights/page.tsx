...
export default async function InsightsPage() {
  ...
  if (IS_DEMO) {
    ...
    // Mock plausible daily/weekly stats from issue totals
    ...
    try {
      ...
      const [{ count: tv }, { count: wv }, { count: tc }, { count: wc }] = await Promise.all([
  ...
  const totalVotes = typedIssues.reduce((sum, i) => sum + i.agree_count + i.disagree_count, 0)
  const totalComments = typedIssues.reduce((sum, i) => sum + i.comment_count, 0)
      ...
      <div className="pt-[72px] pb-[84px] px-4 space-y-3">
            ...
            <div className="rounded-2xl overflow-hidden" style={{ background: 'linear-gradient(135deg, #0038A8 0%, #1a56d6 60%, #003b93 100%)' }}>
              <div className="p-4">
                <div className="flex items-center gap-1.5 mb-2">
                  ...
                  <span className="text-[11px] font-bold text-white/70 tracking-widest uppercase">오늘 하루 핫 이슈</span>
                </div>
                <p className="text-[16px] font-black text-white leading-snug mb-3">
                  {hotIssue.title}
                </p>
        ...
        <div className="grid grid-cols-2 gap-3">
          <div className="bg-white rounded-2xl p-4 border border-gray-100">
            ...
            <div className="text-[24px] font-black text-[#0038A8]">{formatNumber(totalVotes)}</div>
            <div className="text-[11px] text-gray-400 mt-0.5">명이 참여</div>
          </div>
          <div className="bg-white rounded-2xl p-4 border border-gray-100">
            <div className="text-[11px] text-gray-400 mb-1">누적 댓글수</div>
            <div className="text-[24px] font-black text-[#C60C30]">{formatNumber(totalComments)}</div>
              ...
              >
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-[13px] font-semibold text-[#1C1917] line-clamp-1 mb-2">
                      {issue.title}
                    </p>
